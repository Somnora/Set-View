...
export function simulateAiContinuityBreakdown(scene: SceneData): string {
  ...
  // The title says "AI" because that is the feature this stands in for, so the
  // provenance line sits directly under it and travels with the text. This function
  // never contacts a model: it is a deterministic offline heuristic over the scene
  // graph. Without the disclaimer the output is indistinguishable from a real Gemini
  // report once it has been exported to HTML or pasted into a document, and the UI
  // renders it verbatim when no API key is set.
  lines.push(`# AI Shot Continuity & Storyboard Analysis: ${scene.name}`);
  lines.push('');
  lines.push(
    '> **Offline preview, not model output.** Generated by SetView\'s deterministic ' +
      'rule-based continuity heuristic, with no call to Gemini or any other model. ' +
      'Add an API key in the AI Assistant panel for a real model analysis.',
  );
  lines.push('');
