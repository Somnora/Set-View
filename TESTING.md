...
**Dress/Block modes + palm wheel — on-headset (next QA pass):** look at your RAISED left palm → wheel appears in it, facing you; look away → it hides (no flicker at the boundary); a hand hanging at your side while you look at the floor must NOT summon it. **Hands:** tap sectors with the right index fingertip — tap fires once on the push-in, not while held, re-arms on a short pull-back; a side approach onto a sector also works; while the wheel is up, pinches press the hovered sector and NOTHING else (never a placement behind the palm). **Placement:** fresh session starts with Place: Off — pinching anywhere does nothing but select; arm Place: Actor on the wheel, pinch the floor ring → ONE actor appears and the tool disarms (wheel again for the next); with controllers the tool stays armed across trigger pulls, X cycles Off/Actor/Cam. **Controllers:** point + trigger as before. Hub at root flips Block/Dress; ▸ sectors open the sub-wheel in place and the hub becomes ◂ Back (Back returns to root; hiding the wheel also resets to root). Lens ▸ shows live T-stop/format values; Marks ▸ "Mark here" captures a keyframe (hands parity for B). In Dress the grip grabs furniture but never actors (and the reticle stops offering placement); in Block pointing near a couch never hovers it; wheel More pins/unpins the detail panel; entering Dress with a hidden scan auto-ghosts it.
