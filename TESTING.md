...
## QA findings — fourth on-headset session (2026-07-11, James, Quest 3 + Samsung/Android XR)

- [x] **Total lockout: no menu at all, no function worked** — "as soon as I jump in I see a circle [the reticle] but no controls I can access." The gaze-summoned palm wheel never appeared on either device, so there was no path into ANY feature (place, camera, lens, scan). Root cause: the whole menu depended on one fragile signal (looking at your palm within tight dot/distance thresholds). Fixed (pending re-QA) by removing the gamble entirely and giving THREE redundant, foolproof paths:
  1. **The tool wheel now ALWAYS rides the left hand** whenever that hand is tracked and near the head — no gaze, no gesture. Raise your hand and it's there; drop it and it falls out of frame on its own. (`main.ts` loop; gaze code deleted from `wheel.ts`.)
  2. **Hard menu button: Y (left) parks the wheel head-locked in front of your face** (`toggleMenuInFront`) and hides it again — a hand-free path for controller users, exactly the "keybind a button to open the menu" ask.
  3. **Operate it by pointing the right hand/controller at a sector and pulling the trigger / pinching** — the robust ray+pinch path, no occluded-fingertip tap required (fingertip tap still works as a bonus).
  A pinch is a menu press ONLY when the pointer is actually on the wheel (`pointerOnWheel`), so placement/selection still work everywhere else even though the wheel is almost always visible. The wheel is hidden from the virtual-camera pass so it never shows in photos/takes. Y no longer cycles view (that lives on the wheel's View sector); the guide and README/cheat-sheet updated. **On-headset:** confirm the wheel is visible on the left hand the instant you enter, on BOTH the Quest and the Samsung headset; Y pops it in front and hides it; pointing + trigger/pinch presses sectors; placing still works when NOT pointing at the wheel.

## QA findings — third on-headset session (2026-07-10, James, Quest 3, hands)

- [x] **Every fingertip tap spawned another actor** — on hand tracking a palm tap doubles as a system pinch (thumb-index contact), pinch = trigger, and an un-swallowed trigger's DEFAULT meaning was "place an actor at the reticle". The tap-suppression guard (`tipOnWheel`) failed exactly during taps because the tapping finger is occluded behind the palm and its tracked tip drifts off the disc. Fixed (pending re-QA), structurally: **placement is now an armed tool, never a default** — `PlaceArm` starts at `none` every session; a bare pinch/trigger can only *select*, it can never create. Arm from wheel **Place** (Off → Actor → Cam) or **X**. Hands place ONE per arm then auto-disarm (stray pinches are constant on hands); controllers stay armed. Additionally, **while the wheel is visible every pinch is a menu press** — it presses the hovered sector or is swallowed, and never reaches the world.
- [x] **Wheel taps didn't register** — three compounding causes fixed: (1) the press edge required a clean 2.5 cm head-on approach through a narrow "arm band"; a natural side tap never armed. `touchWheel` now arms anywhere NEAR the disc (2× radius) and the press plane is prouder (press 0.012 / re-arm lift 0.025). (2) The disc re-billboarded to the eyes every frame, shifting its local depth axis under the finger mid-tap — it now freezes while a fingertip is on it. (3) When the occluded tip lost tracking there was no fallback — the pointing ray now hovers the wheel too, so a pinch presses the pointed-at sector even when touch fails.
- [x] **False summons while aiming at the floor** — a hand hanging at your side sits inside the gaze cone when you look down (to place/teleport); with the new swallow rule that would eat real actions. The gaze gate now also requires the palm to be RAISED (wrist no more than 0.55 m below the eyes, `GAZE_MAX_DROP_M`), and the summon thresholds loosened slightly (0.82 show / 0.70 hide).
...
- [x] **Hand tool wheel** — `test/wheel-smoke.html` renders the `WheelPanel` in both modes and drives BOTH input paths: a controller raycast (hover + press) and a simulated fingertip tap (hover → push-through), asserting press routing for each: `WHEEL-SMOKE PASS (7 root sectors, ray + touch)`. Pure half (`wheel.ts`: mode-aware sector sets, ring hit-testing with 12-o'clock wrap, armed-placement cycle, fingertip edge detection) in domain tests. (The wheel is no longer gaze-summoned — it always rides the left hand + a hard Y menu button; see the fourth-QA note above.)

**Dress/Block modes + tool wheel — on-headset (next QA pass):** the wheel is ALWAYS on your left hand (raise it to see it, facing you); Y (left controller) also parks it in front of your face and hides it again. **Operate it:** point your right hand/controller at a sector and pull the trigger / pinch (fingertip tap on the disc also works). While the pointer is ON the wheel a pinch presses the sector and nothing else; point away and pinch to act on the world. **Placement:** fresh session starts with Place: Off — pinching anywhere does nothing but select; arm Place: Actor on the wheel, pinch the floor ring → ONE actor appears and the tool disarms (wheel again for the next); with controllers the tool stays armed across trigger pulls, X cycles Off/Actor/Cam. Hub at root flips Block/Dress; ▸ sectors open the sub-wheel in place and the hub becomes ◂ Back (Back returns to root; hiding the wheel also resets to root). Lens ▸ shows live T-stop/format values; Marks ▸ "Mark here" captures a keyframe (hands parity for B). In Dress the grip grabs furniture but never actors (and the reticle stops offering placement); in Block pointing near a couch never hovers it; wheel More pins/unpins the detail panel; entering Dress with a hidden scan auto-ghosts it.
